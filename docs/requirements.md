...
- **Pull request follow-through:** Claude Code watches every pull request it opens until that pull request is merged or closed, and drives it to a green, mergeable state without the owner having to report failures. A failing check is diagnosed from the job's own log before any fix is proposed, never from a guess about the cause. A fix is validated with the repository's checks before it is pushed; when a session cannot run a check (there is no container runtime in a session, and GitHub Actions does not run there), the pull request says which check could not be run locally rather than claiming a verification that did not happen.
- **Green is never bought:** a check is never weakened to make a pull request pass. No test is skipped, disabled or quarantined, no job is made non-blocking, no severity threshold is lowered, and no commit bypasses the checks. When the cause of a failure is ambiguous, or the fix would change a design decision or widen the pull request beyond its topic, Claude Code stops and asks the owner instead of pushing a guess.
- **Merge authority:** Claude Code opens pull requests, pushes to them and answers review comments on them; it never approves and never merges. The owner merges.
...
- Development domains, named after the GCP project that serves them, so that a host says which project answers on it. The production names are a separate decision, taken when the production project is created.
  ...
  - Console: `console.marketplace.lab.aleogr.dev`, a subdomain of its own that serves the staff's sign-in and the console and no marketplace, while the platform and marketplace hosts never serve the console (decided with the owner on 2026-09-26; `docs/superpowers/specs/2026-09-26-f15-console-design.md`, D1)
...
- **Failed second factors:** consecutive failed second-factor answers are counted per account (the owner's decision of 2026-09-25, confirmed for the step-up on 2026-09-26): at 10 the person is told by e-mail, and at the 100th consecutive failure codes from an authenticator app or by e-mail are refused, at sign-in and at the step-up, until the password is changed or a security key or a recovery code is accepted (either lifts it); security keys and recovery codes still work meanwhile.
- **Recovery:** one-time recovery codes are generated when an authenticator app or a security key is enrolled; a staff account is recovered only by the owner or another administrator resetting its factor, with an audit record, which e-mails the staff member a single-use link valid for 24 hours: only through that link, and with the password too, is the new factor enrolled, and a staff sign-in with the password alone never leads to enrolment (the owner's decision of 2026-09-26); stores and buyers recover through e-mail with a security delay (default 24 hours, a parameter), a notification on every channel and revocation of all sessions.
...
- **E-mail: the provider is Brevo.** Amazon SES and Resend also fit the cost premise and either could replace it; Brevo was chosen because its free tier (300 messages a day) covers the platform until it has revenue, it needs no separate AWS account and no sandbox exit, and its transactional API is two endpoints an adapter speaks directly. It **does not sign its webhooks**, so its events are received with a shared token and then **re-read from its API before anything is acted on**, which is the rule this section already states for providers that only send a shared token. Bounce and complaint events become internal events that suppress further sends to that address (see [section 17](#17-notifications)); a soft bounce suppresses nobody, because it is a full mailbox or a busy server and the provider retries it. The suppression list is the platform's and not a marketplace's: an address that reported one marketplace's mail as spam must not receive another's, because the reputation being protected belongs to the sending domain. The sending domain of each marketplace needs DKIM, SPF and DMARC records in Cloudflare and a verified sender at the provider, which is an infrastructure step of marketplace creation.
