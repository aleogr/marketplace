...
16. **Marketplace creation flow** (§29): a console wizard: data and languages → root categories → revenue model → parameters copied from a template → infrastructure checklist executed one step at a time with an instruction for the owner (DNS in Cloudflare, domain mapping, e-mail domain records and production access). The marketplace leaves "in preparation" only when the host answers and the e-mail domain verifies.
...
| Create the e-mail provider account (SES or Resend), verify the platform domain, request production access | Foundation phase, because sign-up and password recovery depend on e-mail |
